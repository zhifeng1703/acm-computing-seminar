...
many mechanisms to allow for the [[https://en.wikipedia.org/wiki/Object-oriented_programming][object-oriented programming paradigm]]. This 
...
#+include: ./src/vector/init/vector.h src cpp -n
...
=const=. Functions with such a signature are not allowed to modify member 
variables, and they are also not allowed to call other non =const= member 
functions.

*** The implementation file
Create the implementation file, =vector.cpp=:

#+include: ./src/vector/init/vector.cpp src cpp -n
...
#+include: ./src/vector/init/main.cpp src cpp -n
...
class that utilizes dynamic memory allocation, such as our =vector= class.

**** Overloading =operator[]=

In the header file, replace the =element= function declaration with:

#+include: ./src/vector/operator-braces/vector.h :lines "16-18" src cpp -n

and in the implementation file, replace the =element= implementation with:

#+include: ./src/vector/operator-braces/vector.cpp :lines "21-27" src cpp -n
...
#+include: ./src/vector/operator-braces/main.cpp :lines "7-9" src cpp -n
...
#+include: ./src/vector/operator-equal/vector.h :lines "19-21" src cpp -n

and let's add to the implementation file,

#+include: ./src/vector/operator-equal/vector.cpp :lines "28-43" src cpp -n

Now, let's assume the =vector= instance, =v=, from above is still defined, and
we'll create a new vector:

#+include: ./src/vector/operator-equal/main.cpp :lines "10-13" src cpp -n
...
#+include: ./src/vector/copy-construct/vector.h :lines "10-12" src cpp -n

and let's add its implementation:

#+include: ./src/vector/copy-construct/vector.cpp :lines "13-22" src cpp -n

Now we compile and run something like:

#+include: ./src/vector/copy-construct/main.cpp :lines "10-12" src cpp -n
...
functions of by labeling them as =friend=. This is useful in situations where 
it is not clear that function should be "called on" an object, i.e. 
=object.method(params)=. Friend functions should be declared in the class 
declaration, but their implementation is not prepended with =class::=, which 
is necessary for member functions.

**** Overloading =operator*=

As an example, we'll overload =operator*= to implement scalar multiplication.

Because scalar multiplication should commute, let's add the following
two declarations to our header file:

#+include: ./src/vector/operator-star/vector.h :lines "25-27" src cpp -n

Let's implement the first by adding to the declaration file:

#+include: ./src/vector/operator-star/vector.cpp :lines "54-63" src cpp -n

Observe how we didn't prepend =vector::= before =operator*= because 
these are =friend= functions.

Now, we can use the first implementation to achieve the second:

#+include: ./src/vector/operator-star/vector.cpp :lines "64-67" src cpp -n

And let's try it out:

#+include: ./src/vector/operator-star/main.cpp src cpp -n
...
Other binary arithmetic operators could also be implements as =friend= 
...
=operator*= for a third time but would accept two vectors as function 
arguments.

**** Overloading =operator<<=
We may overload =operator<<= to send things to the output stream. This 
is a more C++ way to print than our current =print= function. We overload
this operator by adding the following =friend= declaration to the header:

#+include: ./src/vector/operator-ostream/vector.h :lines "30-31" src cpp -n

Note that the =ostream= object belongs to the =std= namespace. Next, we
add to the implementation file:

#+include: ./src/vector/operator-ostream/vector.cpp :lines "68-72" src cpp -n

So now we stream the vector to =std::cout= just as we do for printing 
numbers and strings to the screen:

#+include: ./src/vector/operator-ostream/main.cpp src cpp -n
...
#+include: ./src/matrix/init/matrix.h src cpp -n

Observe how the class declaration begins with the statement, =template<class T>=.
When new instances of the class, =matrix=, are instantiated, the type, =T=, must 
be specified. Notice that the private variable now holds a pointer to type, =T=, 
and the element accessor returns a reference to to type, =T=. Compare this to 
our previous =vector= class where each vector instance held type, =double=.
...
of components from the =matrix= instance, =src=. To use this type of syntax (i.e., 
initializer lists) you may have to compiel with the flag ~-std=c++11~, e.g.,
=g++ -std=c++11 main.cpp=.

Finally, observe that rather the keyword, =private=, we have used the keyword 
=protected=. This will allow inherited classes to use the private member variables 
and functions. We will talk about inheritance later, but for now, it essentially
the same as marking these things as =private=.

Here is some example usage:

#+include: ./src/matrix/init/main.cpp src cpp -n
...
Let us overload =operator*= to implement matrix multiplication. It would also
make sense to overload =operator*= with other call signatures to implement scalar 
...
#+include: ./src/matrix/operator-star/matrix.h :lines "68-97" src cpp -n
...
=template<class T>=. This isn't necessary when the functions were declared 
inside of the class body (i.e., member or friend functions) since the class 
declaration already begins with =template<class T>=.
...
#+include: ./src/matrix/operator-star/main.cpp src cpp -n
...
Let's create another couple of vector classes, called =rowvec= and
=colvec= in a file called =vectors.h=:

#+include: ./src/inheritance/init/vectors.h src cpp -n

We have defined two classes in the =vec= namespace, =rowvec= and =colvec=.
Notice that the declaration of the =rowvec= class is slightly augmented,
...
This says that the =rowvec= class inherits all the methods and
variables defined for the =matrix= class. The keyword =protected=, 
rather than =private= in the =matrix= class allows derived classes 
access to these variables and functions. The keyword, =public=, in 
the class declaration above signals that every =rowvec<T>= instance 
may be cast as a =matrix<T>= instance. This means that functions (such
as our =operator*= in the =mtx= namespace) which expect =matrix= types
in argument, can also now accept =rowvec= types: =rowvec= instances 
will be cast as =matrix= instances when possible.


Next, observe that the constructor simply calls the parent class 
constructor, with an argument of =1= for the number of rows. We also 
add a =len()= function which returns `n_cols()`, calling the parent 
class function. =len()= is more natural for a vector class.

Finally, =operator()= is overwritten so that only a single argument is 
necessary. In the body, we simply call the parent class method. We also 
add =operator[]=, which duplicates the functionality of =operator()=.

Notice that any other methods that exist for the parent class, =matrix=, 
...
In the same, =vec=, namespace we have declared a =colvec= class which 
is analogous to the =rowvec= class except with the dimensions swapped.

Let's check out an example:

#+include: ./src/inheritance/init/main.cpp src cpp -n

This prints:

: 1, 1 ... -30

So =operator*= works by casting both the =rowvec= and =colvec= arguments 
as =matrix= types. The dimensions of these matrices match for matrix 
...
=rowvec*colvec= is performed. Add the inside the =vec= namespace but outside 
the class declarations:

#+include: ./src/inheritance/operator-star/vectors.h :lines "42-54" src cpp -n

Observe that we can call the =operator*= function in the =mtx= namespace,
just like any other function. Now, the product between a =rowvec= and a =colvec=
is more natural:

#+include: ./src/inheritance/operator-star/main.cpp src cpp -n
